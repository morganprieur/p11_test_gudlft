...
# - number of authorized places 
# - /purchasePlaces -> error if book > places 
...
def test_max_points_club(app, client): 
	...
	# assert data['places'] <= club['points'] 
	url = 'booking.html' 
	# --> ok 
	# récupérer les données du form de co pour utiliser la fonction de server.py ??? 
	# assert render_template(url, club=club, competitions=competitions) 
